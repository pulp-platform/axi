...
AXI 디멀티플렉서와 반대 기능을 수행하는 것이 AXI 멀티플렉서입니다. 여러 AXI-4 연결을 병합하여 하나로 합칩니다. 모듈의 여러 슬레이브 포트로부터 들어오는 요청들은 인터리브되어 하나의 마스터 포트를 통해 전송됩니다.

![AXI 4 멀티플렉서 모듈의 블록 다이어그램.](axi_mux.png  "AXI 4 멀티플렉서 모듈의 블록 다이어그램.")

멀티플렉서 모듈은 이전 섹션에서 소개한 디멀티플렉서보다 더 단순한 구조를 가지고 있습니다. AW 및 AR 채널의 요청들은 디멀티플렉서에서 응답을 병합할 때 사용되는 것과 동일한 라운드 로빈 중재 방식으로 병합됩니다. 그러나 한 가지 중요한 차이점은 멀티플렉서가 요청이 어느 슬레이브 포트에서 왔는지를 판단하는 메커니즘입니다. 멀티플렉서는 요청의 `axi_id` 필드의 상위 비트를 이 용도로 사용합니다. 필요한 비트 수는 다음과 같이 계산할 수 있습니다:
...
멀티플렉서에서 스위칭에 ID 프리픽스를 사용하면 이 모듈이 이러한 서로 다른 종류의 명령들 사이의 순서 요구사항을 처리하지 않아도 됩니다. 그 책임은 멀티플렉서의 슬레이브 포트에 연결된 마스터 모듈에게 위임됩니다.
