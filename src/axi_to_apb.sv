...
module axi_to_apb #(
  ...
  logic [BankIdxWidth-1:0] bank_sel, bank_sel_q, bank_sel_d;
  logic                    no_req;
  logic                    dec_err_q, dec_err_d;
  ...
  // for free (no wasted cycles).
  cc_lzc #(
    .Width ( NumBanks                      ),
    .Mode  ( cc_pkg::LZC_TRAILING_ZERO_CNT ) // count trailing zeros -> lowest set bit
  ) i_bank_sel (
    .in_i    ( mem_req  ),
    .cnt_o   ( bank_sel ),
    .empty_o ( no_req   )
  ...
  logic     apb_dec_valid;
  sel_idx_t apb_sel_idx, apb_sel_idx_q, apb_sel_idx_d;
  ...
  always_comb begin
    ...
    unique case (apb_state_q)
      Setup: begin
        if (!no_req) begin
          bank_sel_d    = bank_sel;
          apb_sel_idx_d = apb_sel_idx;
          dec_err_d     = !apb_dec_valid;
          apb_state_d   = Access;
      ...
      Access: begin
        if (dec_err_q) begin
          ...
          mem_rvalid[bank_sel_q] = 1'b1;
          mem_err[bank_sel_q]    = 1'b1;
          apb_state_d            = Setup;
        end else begin
          // ACCESS phase: assert psel and penable until the slave is ready.
          apb_req_o[apb_sel_idx_q] = '{
            paddr:   apb_addr_t'(mem_addr[bank_sel_q]),
            pprot:   mem_prot[bank_sel_q],
            psel:    1'b1,
            penable: 1'b1,
          ...
          if (apb_resp_i[apb_sel_idx_q].pready) begin
            mem_gnt[bank_sel_q]    = 1'b1;
            mem_rvalid[bank_sel_q] = 1'b1;
            mem_rdata[bank_sel_q]  = apb_resp_i[apb_sel_idx_q].prdata;
            mem_err[bank_sel_q]    = apb_resp_i[apb_sel_idx_q].pslverr;
            mem_exokay[bank_sel_q] = 1'b0; // APB has no exclusive access
            apb_state_d            = Setup;
          end
        end
      end
  ...
  `FF(apb_state_q, apb_state_d, Setup, clk_i, rst_ni)
  `FF(bank_sel_q, bank_sel_d, '0, clk_i, rst_ni)
  `FF(apb_sel_idx_q, apb_sel_idx_d, '0, clk_i, rst_ni)
  `FF(dec_err_q, dec_err_d, 1'b0, clk_i, rst_ni)
