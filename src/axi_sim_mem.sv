...
module axi_sim_mem #(
  ...
  initial begin
    assert (AddrWidth != 0) else $fatal(1, "AddrWidth must be non-zero!", 1);
    assert (DataWidth != 0) else $fatal(1, "DataWidth must be non-zero!", 1);
    assert (IdWidth != 0) else $fatal(1, "IdWidth must be non-zero!", 1);
    assert (UserWidth != 0) else $fatal(1, "UserWidth must be non-zero!", 1);
  end
